...
SQL_STRING = f"DELETE FROM {REDSHIFT_DATABASE}.{REDSHIFT_SCHEMA}.{REDSHIFT_TABLE};"
...
def transform(**context):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        results = [
            item
            for sublist in executor.map(process_row, range(len(df)))
            for item in sublist
        ]
...
transform = PythonOperator(
    task_id="transform", python_callable=transform, provide_context=True, dag=dag
)

create_table = PostgresOperator(
    task_id="create_table",
    postgres_conn_id="netproj_redshift_conn_id",
    sql=SQL_STRING+CREATE_TABLE_SQL_STRING,
    dag=dag,
)
